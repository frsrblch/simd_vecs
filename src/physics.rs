...
AddAssign<VMul<'a, Typed<VEC2, UNIT2>, Typed<VEC3, UNIT3>>> for Typed<VEC1, UNIT1>
where
    &'a VEC2: Mul<&'a VEC3>,
...
impl<'a, VEC1, UNIT1, VEC2, UNIT2> Mul<&'a Typed<VEC2, UNIT2>> for &'a Typed<VEC1, UNIT1> {
    type Output = VMul<'a, Typed<VEC1, UNIT1>, Typed<VEC2, UNIT2>>;
...
mod tests {
    ...
    #[test]
    fn add_assign_typed_vecs_and_float() {
        let mut position: Typed<Vec2<f32>, Meters> = Typed {
            vec: Vec2 {
                x: vec![1.0, 2.0].into(),
                y: vec![3.0, 5.0].into(),
        ...
        let time: Typed<f32, Seconds> = Typed {
            vec: 2.0,
            marker: PhantomData,
        };
